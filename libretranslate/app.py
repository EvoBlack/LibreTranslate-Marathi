...
try:
  ...
  _HAS_ARGOSFILES = True
except Exception:
  # Optional dependency not available. Provide a safe fallback so the server
  # can start without file-translation support.
  argostranslatefiles = None
  _HAS_ARGOSFILES = False
...
# Optional Hugging Face adapter for models placed under ./models/
...
def create_app(args):
  ...
  # If there are no installed languages (e.g. models were skipped on this host),
  # avoid crashing by providing a minimal fallback language. Translations will
  # be unavailable until models are installed, but the web service can start.
  if not languages:
    print("WARNING: No installed translation models found. Starting with fallback 'en' language; translations will be unavailable until models are installed.")

    class _FallbackLang:
      def __init__(self, code):
        self.code = code
        self.translations_from = []
        self.to_lang = type('T', (), {'code': code})

    # Create at least two fallback languages to ensure we have fallback options
    languages = [_FallbackLang('en'), _FallbackLang('en')]

  # If a local HF Marathi model exists, expose 'mr' support in the API even if
  # Argos packages for Marathi are not installed. Actual translation will use
  # the HF adapter and we deliberately return None for get_translation so the
  # HF path is preferred.
  try:
    from types import SimpleNamespace
    # _MODEL_DIR resolves to ./models/en-mr-marianmt
    if hasattr(hf_adapter, '_MODEL_DIR') and hf_adapter._MODEL_DIR.exists():
      has_mr = any(getattr(l, 'code', '') == 'mr' for l in languages)
      if not has_mr:
        class _Lang:
          def __init__(self, code, name):
            self.code = code
            self.name = name
            # Argos objects expose translations_from entries with .to_lang.code
            self.translations_from = []
            self.to_lang = SimpleNamespace(code=code)
          def get_translation(self, tgt_lang):
            return None

        mr_lang = _Lang('mr', 'Marathi')
        # Ensure 'en' exists; if not, add a minimal one
        en_obj = next(iter([l for l in languages if getattr(l, 'code', '') == 'en']), None)
        if en_obj is None:
          en_obj = _Lang('en', 'English')
          languages.append(en_obj)

        # Declare mutual direction availability for UI/spec building
        class _Trans:
          def __init__(self, to_lang):
            self.to_lang = to_lang

        mr_lang.translations_from.append(_Trans(en_obj))
        en_obj.translations_from = list(getattr(en_obj, 'translations_from', [])) + [_Trans(mr_lang)]

        languages.append(mr_lang)
  except Exception:
    pass
  ...
  # Map user defined frontend languages to argos language object.
  ...
  if args.req_limit > 0 or args.api_keys or args.daily_req_limit > 0 or args.hourly_req_limit > 0:
    ...
    def health():
        ...
        return jsonify({
            "status": "healthy",
            "version": get_version(),
            "languages": len(languages) if 'languages' in locals() else 0
        })
            ...
            schema:
              ...
              properties:
                error:
                  ...
                  description: Error message
        """
    # If somehow invoked without a proper request context (e.g., during app
    # initialization via tooling that introspects routes), bail out early.
    if not has_request_context():
      return Response('{"error":"Service starting"}', status=503, mimetype='application/json')
    ...
    if request.is_json:
      ...
      num_alternatives = int(request.values.get("alternatives", 0))
    ...
    try:
      if batch:
        ...
        batch_alternatives = []
        for text in q:
          # Prefer HF adapter for supported pairs
          hf_translator = hf_adapter.get_translator(detected_src_lang["language"], target_lang)
          translator = hf_translator or src_lang.get_translation(tgt_lang)
    ...
    def translate_file():
        ...
        if not _HAS_ARGOSFILES:
            abort(503, description=_("File translation is not available on this instance (missing optional dependency)."))
    ...
    @limiter.exempt
    def spec():
        # Build Swagger spec lazily under a request context to avoid touching
        # any request-bound objects at import/startup time.
        _swag = swagger(app)
        _swag["basePath"] = args.url_prefix if args.url_prefix != "" else "/"
        _swag["info"]["version"] = get_version()
        _swag["info"]["title"] = "LibreTranslate"
        _swag["info"]["description"] = "Free and Open Source Machine Translation API."
        _swag["info"]["license"] = {"name": "AGPL-3.0"}
        return jsonify(lazy_swag(_swag))
